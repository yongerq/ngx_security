...
-- 请求方式： http://{host}/upstream/down?upstream_name=aaa.com&upstream_ip=10.23.1.17
...
ngx.log(ngx.ERR, "start upstream peer down.")
...
if nil ~= upstream_name and nil ~= upstream_ip then
    ...
    for key, srv in ipairs(srvs) do
        ...
        if upstream_ip == pureAddr then
            ngx.log(ngx.ERR, "set peer down success.")

            upstream.set_peer_down(upstream_name, backup, key-1, true)
            success = true
...
ngx.log(ngx.ERR, "end upstream peer down.")
